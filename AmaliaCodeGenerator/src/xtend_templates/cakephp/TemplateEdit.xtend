	...
	<nav class="large-3 medium-4 columns" id="actions-sidebar">
		    <ul class="side-nav">
		        ...
		        <li><?= $this->Form->postLink(
		                __('Delete'),
		                ['action' => 'delete', $«entity.name.toLowerCase»->id],
		                ['confirm' => __('Are you sure you want to delete # {0}?', $«entity.name.toLowerCase»->id)]
		            )
		        ?></li>
		...
		<div class="«Inflector.plural(entity.name.toLowerCase)» form large-9 medium-8 columns content">
		    ...
		    <fieldset>
		        <legend><?= __('Edit «entity.name.toLowerCase.toFirstUpper»') ?></legend>
		        <?php
		            «options»
		        ?>
		    </fieldset>
